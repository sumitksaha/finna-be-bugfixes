...
<head>
    ...
    <script src="<?php echo base_url() ?>extra/javascript/waitForImages/waitForImages.js"></script>
			...
			function get_question_set() {
				...
				error : function(XMLHttpRequest, textStatus, errorThrown) {
					if(textStatus == 'timeout') {
						alert('Connection seems dead! Try reloading');
					}
				},
				data : {subject_name : subject_name, set_no : set_no}
					...
					load_Mathjax();
					var start_exam_string = "<span style='cursor: pointer; background-color: #16A2EB; color: #FFFFFF; font-size: 25px; border-radius: 7px; padding-left: 10px; padding-right: 10px; padding-top: 5px; padding-bottom: 5px;'>Click here to start the exam</span>";
					// var timer_div_string = "";
					var submit_button_string = "<span style='cursor: pointer; background-color: #16A2EB; color: #FFFFFF; font-size: 25px; border-radius: 7px; padding-left: 10px; padding-right: 10px; padding-top: 5px; padding-bottom: 5px;'>Submit Answers</span>";
					$('#each_questions').append(string);
					$('#question_nav_element').append(nav_string);
					// $('.timer_div').append(timer_div_string);
					$('#submit_button').append(submit_button_string);
					$('#start_exam').append(start_exam_string);
					// var error_string = '<div style="text-align: center;">All questions loaded.</div>';
					// $('.progress_bar').append(error_string);
					$('.content').hide();
					$('#start_exam').show();
					
					// total_images_loaded = 0;
					// $('#each_questions').waitForImages(function() {
						
					// }, function(loaded, count, success) {
					// 	var error_string = '<div style="text-align: center;">'+loaded + ' of ' + count + ' images has ' + (success ? 'loaded' : 'failed to load') +  '.</div>';
					// 	if(loaded<count) {
					// 		$('.progress_bar').append(error_string);
			...
			<div class="container_block">
				...
				<article class="article">
					...
					<div class="questions" style="display:inline-block;width:100%;float:right;margin-top:40px;">
						...
						<div id='start_exam' style='text-align: center;'></div>
					   ...
					   <!--  <div class='progress_bar'><br /><br /></div> -->
						...
						<div class='timer_div' style='background-color: #E8F6FD; margin-top: 25px; padding-top: 10px;'>
							<span class='prev' style='font-size: medium; float: left; margin-left:30px; cursor: pointer;'>Save & Previous question</span><span class='timer' style='font-size: medium; float: left; margin-left: 210px;'>Time remaining : </span><span class='countdowntime' style='font-size: medium;'>"+<?php echo $time; ?>+":00:00</span><span class='next' style='font-size: medium; float: right; margin-right:30px; cursor: pointer;'>Save & Next question</span><br /><br />
						</div>
						...
						<div><br /><br /></div>
						<div id='submit_button' style="text-align: center;"></div>
						<div id='answer' style="display: inline-block; width: 100%;"></div>
						<div><br /><br /></div>
						<div id='nav_set'>
							<span style='float: left; margin-left: 100px;'><a href="<?php echo base_url().'index.php/ibps_po/'.$subject_name; ?>" rel='nofollow'>Back to All Sets</a></span>
							<span style='float: left; margin-left: 200px;'><a href="<?php echo base_url().'index.php/ibps_po/'.$subject_name.'/set-0'.$set_no; ?>" rel='nofollow'>Try this set again</a></span>
							<span style='float: right; margin-right: 100px;'><a href="<?php echo base_url().'index.php/ibps_po/'.$subject_name.'/set-0'.($set_no+1); ?>" rel='nofollow'>Move to next set</a></span>
						</div>
